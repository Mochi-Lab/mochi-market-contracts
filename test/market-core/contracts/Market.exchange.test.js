...
describe('Market', async () => {
	...
	describe('Alice createExchangeOrder (ERC721 <-> ERC721, ETH) successfully', async () => {
		let erc721A;
		let erc721B;
		let souTokenId = 0;
		let desTokenId = 0;
	...
	describe('Alice createExchangeOrder (ERC721 <-> ERC721, moma) successfully', async () => {
		let erc721A;
		let erc721B;
		let souTokenId = 0;
		let desTokenId = 0;
		...
		it('Check sell order info by getExchangeOrderById', async () => {
			...
			expect(exchangeOrderInfo.tokens[0]).to.be.equal(moma.address);
			expect(exchangeOrderInfo.tokens[1]).to.be.equal(moma.address);
		...
		describe('User calls exchange successfully (without reward token)', async () => {
			...
			beforeEach(async () => {
				...
				await market.connect(bob).exchange('0', '1', bob.address, '0x');
			});
			...
			it('Check balance and ownership', async () => {
				...
				expect(await vault.getRoyalty(erc721A.address, moma.address)).to.be.equal(
					(((price * 10) / 1000) * 20) / 100
				);

				let ethRewardToken = await vault.getRewardToken(moma.address);
				expect(await vault.getRewardTokenBalance(alice.address, ethRewardToken)).to.be.equal(0);
			});
	...
	describe('Alice createExchangeOrder (ERC721 <-> ERC1155, ETH) successfully', async () => {
		let erc721;
		let erc1155;
		let souTokenId = 0;
		let desTokenId = 0;
		...
		beforeEach(async () => {
			...
			await erc1155.connect(deployer).mint(bob.address, desTokenId, 5, '0x');
		...
		it('Check sell order info by getExchangeOrderById', async () => {
			...
			expect(exchangeOrderInfo.nftAmounts[0]).to.be.equal(1);
			expect(exchangeOrderInfo.nftAmounts[1]).to.be.equal(5);
	...
	describe('Alice createExchangeOrder (ERC721 <-> ERC1155, moma) successfully', async () => {
		let erc721;
		let erc1155;
		let souTokenId = 0;
		let desTokenId = 0;
		...
		describe('User calls exchange successfully (without reward token)', async () => {
			...
			it('Check exchangeOrderInfo', async () => {
				...
				expect(await erc721.ownerOf(souTokenId)).to.be.equal(bob.address);
				expect(parseInt(await erc1155.balanceOf(alice.address, desTokenId))).to.be.equal(5);
				expect(parseInt(await erc1155.balanceOf(bob.address, desTokenId))).to.be.equal(0);
	...
	describe('Alice createExchangeOrder (ERC1155 <-> ERC721, ETH) successfully', async () => {
		let erc721;
		let erc1155;
		let souTokenId = 0;
		let desTokenId = 0;
	...
	describe('Alice createExchangeOrder (ERC1155 <-> ERC1155, ETH) successfully', async () => {
		let erc1155A;
		let erc1155B;
		let souTokenId = 0;
		let desTokenId = 0;
