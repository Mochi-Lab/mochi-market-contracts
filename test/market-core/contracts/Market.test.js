...
describe('Market', async () => {
  let addressesProvider, nftListProxy, vaultProxy, marketProxy, sellOrderListProxy;
  ...
  it('Call updateFee failed cause caller is not market admin', async () => {
    await expect(marketProxy.connect(alice).updateFee('3', '1000')).to.be.revertedWith(
  ...
  context('Setup ERC721', async () => {
    beforeEach(async () => {
      ...
      nonAccepetedERC721 = await deployTestERC721(alice, 'Non-Accepted', 'Non-Accepted');
    ...
    it('create sell failed cause nft address has not been accepted', async () => {
      await expect(
        marketProxy
          .connect(alice)
          .createSellOrder(nonAccepetedERC721.address, tokenId, '1', price, ETH_Address)
    ...
    context('Sell order is created successfully', async () => {
      ...
      it('create sell order successfully', async () => {
        ...
        expect(
          await sellOrderListProxy.checkDuplicate_ERC721(testERC721.address, tokenId, alice.address)
        ).to.be.equal(true);
      });
      ...
      it('cancle sell order failed cause caller is not seller', async () => {
        await expect(marketProxy.connect(bob).cancleSellOrder(sellId)).to.be.revertedWith(
          ERRORS.CALLER_NOT_SELLER
        );
      });
      ...
      it('cancle sell order successfully', async () => {
        ...
        rsult = await sellOrderListProxy.getAvailableSellOrdersIdListByUser(alice.address);
        expect(result.length).to.be.equal(0);
      ...
      it('cancle sell order failed cause sellOrder is not active', async () => {
        await marketProxy.connect(alice).cancleSellOrder(sellId);
        await expect(marketProxy.connect(alice).cancleSellOrder(sellId)).to.be.revertedWith(
          ERRORS.SELL_ORDER_NOT_ACTIVE
        );
      });

      it('update price failed cause caller is not seller', async () => {
        await expect(marketProxy.connect(bob).updatePrice(sellId, '2000000')).to.be.revertedWith(
          ERRORS.CALLER_NOT_SELLER
        );
      });

      it('update price failed cause price not change', async () => {
        await expect(marketProxy.connect(alice).updatePrice(sellId, price)).to.be.revertedWith(
          ERRORS.PRICE_NOT_CHANGE
        );
      });
      ...
      it('buy failed cause caller is seller', async () => {
        await expect(
          marketProxy.connect(alice).buy(sellId, '1', alice.address, '0x', { value: price })
      ...
      it('buy failed cause sell order was cancled', async () => {
        ...
        await expect(
          marketProxy.connect(bob).buy(sellId, '1', bob.address, '0x', { value: price })
        ).to.be.revertedWith(ERRORS.SELL_ORDER_NOT_ACTIVE);
      });

      it('buy failed cause msg.value is not equal price', async () => {
        await expect(
          marketProxy.connect(bob).buy(sellId, '1', bob.address, '0x', { value: 999999 })
        ).to.be.revertedWith(ERRORS.VALUE_NOT_EQUAL_PRICE);

        await expect(
          marketProxy.connect(bob).buy(sellId, '1', bob.address, '0x', { value: 1000001 })
        ).to.be.revertedWith(ERRORS.VALUE_NOT_EQUAL_PRICE);
      });

      it('buy successfully', async () => {
        await marketProxy.connect(bob).buy(sellId, '1', bob.address, '0x', { value: price });
        ...
        result = await sellOrderListProxy.getAvailableSellOrdersIdListByUser(alice.address);
